...
from mini_datahub.index import list_all_datasets
...
class AutocompleteManager:
    ...
    def load_from_catalog(self, catalog_path: Optional[Path] = None) -> int:
        ...
        try:
            ...
            # Fallback: load from YAML files if database fails
            if catalog_path:
                count = self._load_from_yaml_files(catalog_path)

        return count

    def _load_from_yaml_files(self, catalog_path: Path) -> int:
        """
        ...
        for metadata_file in data_dir.glob("*/metadata.yaml"):
            try:
                ...
                if 'used_in_projects' in data and isinstance(data['used_in_projects'], list):
                    self.projects.update(p for p in data['used_in_projects'] if p)

                if 'data_types' in data and isinstance(data['data_types'], list):
                    self.data_types.update(self.normalize_data_type(t) for t in data['data_types'] if t)

                if 'file_format' in data:
                    self.file_formats.add(self.normalize_format(str(data['file_format'])))
