...
- 🔐 **Privacy & Security**: Secure credential storage, no Git history, comprehensive security documentation
- ☁️ **Cloud-Native**: Direct WebDAV integration with HeiBox, cloud-only workflow
- ⚡ **Performance**: Dramatic speed improvements (startup <300ms, search <80ms)
- 🎨 **UX Enhancements**: Smart autocomplete, visual badges, better keybindings
- 🛠️ **Developer Tools**: Auth diagnostics, status indicators, dataset management
...
#### `hei-datahub auth doctor` - Deep Diagnostics
Comprehensive authentication and connectivity diagnostics:

**Checks performed:**

- ✅ Config file existence and TOML validity
- ✅ Credentials presence in keyring/env
- ✅ Network reachability (DNS + TCP)
- ✅ WebDAV authentication (PROPFIND)
- ✅ Write permissions (PUT/DELETE test)
- ✅ Clock skew detection
- ✅ Mount path awareness
...
- ❌ Removed local filesystem storage option
- ✅ All datasets saved directly to WebDAV (HeiBox)
- ✅ No more Git commits or pull requests
- ✅ Direct cloud upload on dataset creation
...
- **Two-level completion**:
  ...
  - Type `so` → suggests `source:` (field name)
  - Type `source:` → suggests actual source URLs (field values)

- **Intelligent ranking**:

  - 2.0× weight for prefix matches
  - 1.5× weight for frequency
  - 1.2× weight for recency
  - 0.5× weight for alphabetical order
...
|------------|-------|---------|
| Field filter (`:`) | 🏷 | `🏷 project:ML` |
| Exact match (`=`) | 🎯 | `🎯 type=dataset` |
...
**Codename Origin:** Focus on secure credential storage and privacy-first architecture
...
🔐 **Security**: WebDAV auth with secure keyring storage
☁️ **Cloud**: Direct HeiBox integration, cloud-only workflow
⚡ **Performance**: 10-25x faster startup and search
🎨 **UX**: Smart autocomplete, visual badges, better keybindings
🛠️ **Tools**: Auth diagnostics, status indicators, dataset management
📚 **Docs**: Privacy guide, auth setup guide, updated references
