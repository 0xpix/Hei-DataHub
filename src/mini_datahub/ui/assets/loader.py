...
def get_logo_text(config) -> str:
    ...
    try:
        ...
        try:
            ...
            logo_file = files("mini_datahub.ui.assets.ascii").joinpath("logo_default.txt")
            text = logo_file.read_text(encoding="utf-8")
        except (ImportError, AttributeError):
            # Fallback for older Python
            import pkg_resources
            text = pkg_resources.resource_string(
                "mini_datahub.ui.assets.ascii",
                "logo_default.txt"
            ).decode("utf-8")
...
def get_stylesheets(config) -> list[Path]:
    ...
    try:
        from importlib.resources import files
        base_style = files("mini_datahub.ui.styles").joinpath("base.tcss")
