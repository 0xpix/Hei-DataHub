...
def run_clear(force: bool = False, clear_all: bool = False) -> int:
    ...
    if platform.system() != "Linux":
        print("❌ Error: auth clear is only supported on Linux.", file=sys.stderr)
        return 2
    ...
    try:
        from mini_datahub.auth.credentials import get_auth_store, KeyringAuthStore
    ...
    if clear_all:
        try:
            from mini_datahub.infra.config_paths import get_user_config_dir
