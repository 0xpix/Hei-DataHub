...
Provides local, indexed search for datasets with <20ms queries.
"""
import asyncio
import logging
import os
...
from mini_datahub.infra.paths import CACHE_DIR
...
class IndexService:
    ...
    def get_item_count(self) -> Tuple[int, int]:
        """Get count of items (total, remote)."""
        conn = self.get_connection()
        try:
            cursor = conn.execute("SELECT COUNT(*) as total FROM items")
            total = cursor.fetchone()["total"]

            cursor = conn.execute("SELECT COUNT(*) as remote FROM items WHERE is_remote = 1")
